...
import { getProductsService } from '../../../../common/services/api/products/products.service';
...
export default function ProductsList() {
    ...
    useEffect(() => {
        getProductsService()
        .then((products) => setProductsBo(products))
    },[])
    return (
        <Box padding={"22px"}>
            <Box>
                <Typography variant="h3" fontSize={28} fontFamily={"Poppins"} fontWeight={600} >Produits</Typography>
                <Typography variant="body2" fontSize={12} marginTop={1} fontWeight={400} fontFamily={"Poppins"}>
                    Créez, modifiez ou supprimez autant de produits que vous le souhaitez. Une fois créés, ces produits seront disponibles sur les différentes pages de gestion des menus.
                </Typography>
            </Box>
            <Box sx={{
                marginTop: "40px",
                        ...
                        <Table aria-label="simple table">
                            <TableHead sx={{ backgroundColor: '#F0F0F0' }}>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ color: "#7C7C7C", fontSize: "12px", fontWeight: 500, padding: "10px" }} align="left" >Description</TableCell>
                                    <TableCell sx={{ color: "#7C7C7C", fontSize: "12px", fontWeight: 500, padding: "10px" }} align="left" >Prix </TableCell>
                                    <TableCell sx={{ color: "#7C7C7C", fontSize: "12px", fontWeight: 500, padding: "10px" }} align="left" >Photo</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {productsBo && productsBo.map((row) => (
                                    <TableRow
                                        key={row.id}
                                ...
                                {row.id}
                                        ...
                                        <TableCell align="left" sx={{ paddingX: "3px", paddingY: "10px", maxWidth:"450px" }}>
                                            <Typography sx={{ fontSize: "13px" }}>
                                                {row.description}
                                            </Typography>
                                        </TableCell>
